...
public class AdvertisementServiceImpl extends GenericServiceImpl<Advertisement> implements AdvertisementService {
	...
	public Set<Advertisement> getAdsForUser(@Valid User currentUser, Pageable pageable) {
		return advertisementRepository.findAdsForUser(currentUser.getLocation(), currentUser.getGender(), currentUser.getAuthProvider().equals(AuthProvider.LOCAL) ? LocalDate.now().getYear() - currentUser.getDateOfBirth().getYear() : 13, pageable).toSet();
	}
}
